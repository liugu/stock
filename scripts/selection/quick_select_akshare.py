#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""AkShare 版策略选股 - 使用数据适配器自动切换数据源"""

import pandas as pd
import numpy as np
import warnings
import logging
warnings.filterwarnings('ignore')
logging.basicConfig(level=logging.INFO, format='%(message)s')

# 导入适配器
from instock.core.crawling.data_adapter import get_stock_hist, get_stock_spot, get_zt_pool

def simple_ma(data, period):
    return pd.Series(data).rolling(window=period).mean().values

def calculate_rsi(close, period=14):
    deltas = np.diff(close)
    gains = np.where(deltas > 0, deltas, 0)
    losses = np.where(deltas < 0, -deltas, 0)
    avg_gain = np.convolve(gains, np.ones(period)/period, mode='valid')
    avg_loss = np.convolve(losses, np.ones(period)/period, mode='valid')
    rs = np.where(avg_loss != 0, avg_gain / avg_loss, 100)
    rsi = 100 - (100 / (1 + rs))
    return np.concatenate([[50] * period, rsi])

def calculate_macd(close):
    ema12 = pd.Series(close).ewm(span=12).mean()
    ema26 = pd.Series(close).ewm(span=26).mean()
    dif = (ema12 - ema26).values
    dea = pd.Series(dif).ewm(span=9).mean().values
    macd = (dif - dea) * 2
    return dif, dea, macd

print('=' * 60)
print('A股策略选股 (AkShare 适配版)')
print('=' * 60)

# 获取实时行情
print('\n[1/3] 获取实时行情...')
df_spot = get_stock_spot()

if df_spot.empty:
    print('      获取实时行情失败，尝试使用新浪接口...')
    import requests
    all_data = []
    url = 'http://vip.stock.finance.sina.com.cn/quotes_service/api/json_v2.php/Market_Center.getHQNodeData'
    for page in range(1, 30):
        params = {'page': str(page), 'num': '100', 'sort': 'changepercent', 'asc': '0', 'node': 'hs_a'}
        try:
            r = requests.get(url, params=params, timeout=30)
            data = r.json()
            if not data:
                break
            all_data.extend(data)
        except:
            break
    df_spot = pd.DataFrame(all_data)
    df_spot = df_spot.rename(columns={'code': '代码', 'name': '名称', 'trade': '最新价', 
                                       'changepercent': '涨跌幅', 'amount': '成交额', 
                                       'mktcap': '总市值', 'per': '市盈率', 'pb': '市净率'})
else:
    # AkShare 返回的列名已经是中文
    # 需要统一格式
    if '代码' not in df_spot.columns:
        df_spot = df_spot.rename(columns={'代码': '代码', '名称': '名称'})

print(f'      共获取 {len(df_spot)} 只股票')

# 筛选条件
print('\n[2/3] 筛选候选股票...')
candidates = df_spot[
    (pd.to_numeric(df_spot['最新价'], errors='coerce') > 3) & 
    (pd.to_numeric(df_spot['最新价'], errors='coerce') < 100) &
    (pd.to_numeric(df_spot['成交额'], errors='coerce') > 50000000) & 
    (pd.to_numeric(df_spot['总市值'], errors='coerce') > 200000000) &
    (pd.to_numeric(df_spot['市盈率'], errors='coerce') > 0) & 
    (pd.to_numeric(df_spot['市盈率'], errors='coerce') < 100)
].copy()
print(f'      筛选后剩余 {len(candidates)} 只')

# 获取历史数据并计算策略
print('\n[3/3] 分析策略信号...')
results = []
total = min(len(candidates), 150)

for idx, (_, row) in enumerate(candidates.head(total).iterrows()):
    if idx % 30 == 0:
        print(f'      进度: {idx}/{total}')
    
    code = str(row['代码']).zfill(6)
    name = row['名称']
    
    try:
        # 使用适配器获取历史数据
        hist_df = get_stock_hist(code, period='daily', adjust='qfq')
        
        if hist_df.empty or len(hist_df) < 60:
            continue
        
        close = hist_df['收盘'].values
        
        # RSI
        rsi = calculate_rsi(close)
        
        # MACD
        dif, dea, macd = calculate_macd(close)
        
        # 均线
        ma5 = simple_ma(close, 5)
        ma10 = simple_ma(close, 10)
        ma20 = simple_ma(close, 20)
        ma30 = simple_ma(close, 30)
        
        # 评分
        score = 0
        signals = []
        
        # RSI信号
        if rsi[-2] < 30 and rsi[-1] > rsi[-2]:
            score += 35
            signals.append('RSI超卖回升')
        elif 30 < rsi[-1] < 70 and rsi[-1] > rsi[-2]:
            score += 20
            signals.append('RSI向上')
        
        # MACD信号
        if dif[-2] < dea[-2] and dif[-1] > dea[-1]:
            score += 30
            signals.append('MACD金叉')
        elif dif[-1] > 0 and dea[-1] > 0 and dif[-1] > dif[-2]:
            score += 15
            signals.append('MACD多头')
        
        # 均线信号
        if ma5[-1] > ma10[-1] > ma20[-1] > ma30[-1]:
            score += 25
            signals.append('均线多头')
        elif ma5[-2] < ma20[-2] and ma5[-1] > ma20[-1]:
            score += 20
            signals.append('均线金叉')
        
        if score >= 30:
            results.append({
                '代码': code,
                '名称': name,
                '最新价': float(row['最新价']),
                '涨跌幅': float(row['涨跌幅']),
                '市盈率': float(row['市盈率']),
                '综合得分': score,
                '信号': ', '.join(signals)
            })
    except Exception as e:
        continue

# 输出结果
print('\n' + '=' * 60)
print('选股结果')
print('=' * 60)

if results:
    results_df = pd.DataFrame(results).sort_values('综合得分', ascending=False)
    print(f'\n共找到 {len(results_df)} 只符合条件的股票，展示前10只:\n')
    
    for i, (_, row) in enumerate(results_df.head(10).iterrows(), 1):
        print(f"【{i}】{row['代码']} {row['名称']}")
        print(f"    价格: {row['最新价']:.2f}元 | 涨跌: {row['涨跌幅']:.2f}% | PE: {row['市盈率']:.1f}")
        print(f"    得分: {row['综合得分']}分 | 信号: {row['信号']}")
        print()
    
    # 保存结果
    output_file = f"策略选股_akshare_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    results_df.to_excel(output_file, index=False)
    print(f"结果已保存: {output_file}")
else:
    print('\n未找到符合条件的股票')

print('=' * 60)