...
for target in ['2026-07-17', '2026-07-20']:
    ...
    for bi in range(0, len(all_stocks), BATCH):
        ...
        print(f'  {target} 批次{bi//BATCH+1}: {done}/{len(all_stocks)} | 成功{success} | {elapsed:.0f}s')
    
    cur.execute('SELECT COUNT(DISTINCT stock_id) FROM stock_daily WHERE date = %s', (target,))
    cnt = cur.fetchone()[0]
    print(f'{target}: {cnt}/{len(all_stocks)} ({cnt/len(all_stocks)*100:.1f}%)')
    
    cur.close()
    conn.close()
    if bs_login(): bs.logout()
